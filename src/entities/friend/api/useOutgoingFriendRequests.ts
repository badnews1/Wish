/**
 * Хук для получения исходящих запросов в друзья
 */
...
interface UseOutgoingFriendRequestsParams {
  userId: string;
}
 ...
 */
export function useOutgoingFriendRequests({ userId }: UseOutgoingFriendRequestsParams) {
  return useInfiniteQuery({
    queryKey: ['outgoing-friend-requests', userId],
    enabled: !!userId,
    
    queryFn: async ({ pageParam }) => {
      const cursor = pageParam as FriendsPaginationParams['cursor'] | undefined;

      // RPC функция для исходящих запросов
      const { data, error } = await supabase
        .rpc('get_outgoing_friend_requests_paginated', {
          p_user_id: userId,
          p_limit: PAGE_SIZE,
