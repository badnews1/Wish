...
-- ⚠️ ВАЖНО: Эта миграция дополняет 20260117_create_friendships_table.sql
...
-- НОРМАЛИЗАЦИЯ: Поменять местами user_id и friend_id где user_id > friend_id
-- (для существующих записей, чтобы constraint не сломался)
UPDATE friendships
SET 
  user_id = friend_id,
  friend_id = user_id
...
ALTER TABLE friendships DROP CONSTRAINT IF EXISTS normalized_pair;
...
-- RLS для friend_actions: удалить старую политику если есть
DROP POLICY IF EXISTS "No direct access" ON friend_actions;
...
COMMENT ON COLUMN friendships.status IS 'Статус дружбы: pending (ожидание), accepted (принято)';
